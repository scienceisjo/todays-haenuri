...
function render(){
  ...
  root.innerHTML=`<div class="shell"><aside class="sidebar"><div class="brand"><img src="${logoSrc(S.data)}" alt=""><span>${h(appName)}</span></div>
    ...
    <div class="sidebar-bottom"><div class="metadata">교직원을 위한<br>우리 학교의 하루</div><div class="sidebar-line"></div><div class="account"><div class="avatar">${h(S.user.name[0])}</div><div><div class="small">${h(S.user.name)}${isShared()?'':' 선생님'}</div><div class="metadata">${isShared()?'전 교직원 공용 계정':h(S.data.departments.find(d=>d.id===S.user.department_id)?.name||'교직원')}</div></div></div><button class="ghost btn-icon" data-action="autostart">${icon('clock')}<span>PC 켜면 자동 실행</span></button><button class="ghost btn-icon" data-action="install-app" id="install-btn" ${window.__installPrompt?'':'hidden'}>${icon('download')}<span>바탕화면 앱으로 설치</span></button>${isShared()?'':`<button class="ghost btn-icon" data-action="password">${icon('settings')}<span>비밀번호 변경</span></button>`}<button class="ghost btn-icon" data-action="logout">${icon('logout')}<span>로그아웃</span></button></div></aside>
...
function autostartDialog(){openDialog('PC 켜면 자동 실행',`<p class="notice-small">선생님 PC에 로그인하면 이 앱이 저절로 열리게 합니다. 파일 하나 내려받아 실행하면 끝(관리자 권한 필요 없음, 시작 프로그램에 바로가기 하나 추가).</p>
  <ol class="steps"><li><a class="btn-link" href="./autostart-on.bat" download>autostart-on.bat 내려받기 →</a> 내려받은 파일을 두 번 클릭해 실행합니다. 브라우저가 “안전하지 않을 수 있는 파일”이라 하면 <b>유지</b>, Windows가 물으면 <b>실행</b>을 누르세요.</li><li>앱이 바로 한 번 열립니다. 로그인 화면의 <b>“이 컴퓨터에서 로그인 유지”</b>에 체크해 두면 다음부터는 로그인 없이 바로 대시보드가 뜹니다.</li><li>끄고 싶을 때는 <a class="btn-link" href="./autostart-off.bat" download>autostart-off.bat 내려받기 →</a> 실행.</li></ol>
  <details class="mt"><summary class="small">다른 방법 (설치한 앱의 설정으로)</summary><p class="small muted mt-s">‘바탕화면 앱으로 설치’를 한 뒤 — Edge: 주소창 오른쪽 ⋯ 메뉴 → 앱 → 앱 관리 → 오늘의 해누리 → <b>로그인 시 앱 시작</b> 켜기. Chrome: 주소창에 chrome://apps → 앱 아이콘 오른쪽 클릭 → <b>로그인 시 앱 시작</b>.</p></details>
  <p class="metadata mt">파일이 하는 일: 시작 프로그램 폴더(shell:startup)에 Edge/Chrome 을 앱 창(--app)으로 여는 바로가기 ‘Todays-Haenuri’를 만듭니다. 브라우저를 못 찾으면 기본 브라우저용 인터넷 바로가기를 만듭니다.</p>`,`<button data-action="close">닫기</button>`);}
...
document.addEventListener('click',async event=>{
  ...
  try{
    ...
    if(action==='autostart')return autostartDialog();
