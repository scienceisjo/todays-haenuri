...
function render(){
  ...
  root.innerHTML=`<div class="shell"><aside class="sidebar"><div class="brand"><img src="${logoSrc(S.data)}" alt=""><span>${h(appName)}</span></div>
    ...
    <div class="sidebar-bottom"><div class="metadata">교직원을 위한<br>우리 학교의 하루</div><div class="sidebar-line"></div><div class="account"><div class="avatar">${h(S.user.name[0])}</div><div><div class="small">${h(S.user.name)} 선생님</div><div class="metadata">${h(S.data.departments.find(d=>d.id===S.user.department_id)?.name||'교직원')}</div></div></div><button class="ghost btn-icon" data-action="install-app" id="install-btn" ${window.__installPrompt?'':'hidden'}>${icon('download')}<span>바탕화면 앱으로 설치</span></button><button class="ghost btn-icon" data-action="password">${icon('settings')}<span>비밀번호 변경</span></button><button class="ghost btn-icon" data-action="logout">${icon('logout')}<span>로그아웃</span></button></div></aside>
