...
export function createData(config, supabaseLib) {
  ...
  const db = {
    ...
    admin: {
      ...
      async roster() { return ok(await t('roster').select('email,name,department_id,role,added_at').order('name')); },
      async addRoster(rows) { requireMe(); ok(await t('roster').upsert(rows.map(r => ({ email: String(r.email).trim().toLowerCase(), name: String(r.name).trim(), department_id: r.department_id || null, role: r.role === 'admin' ? 'admin' : 'staff', added_by: me.id })), { onConflict: 'email' })); return { count: rows.length }; },
      async removeRoster(email) { ok(await t('roster').delete().eq('email', email)); return { ok: true }; },
      ...
      async exportAll() {
        ...
        for (const name of ['departments', 'staff', 'roster', 'records', 'record_series', 'attachments', 'meals', 'meal_duties', 'holidays', 'settings']) out[name] = ok(await t(name).select('*'));
        return out;
      },
